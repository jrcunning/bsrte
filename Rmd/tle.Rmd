...
plots <- tle %>%
  ...
  nest() %>%
  full_join(pp) %>%
  mutate(plot = pmap(list(d = data, p = pred, n = nursery, t = tree), function(d, p, n, t) {
    ggplot(d, aes(x = date, y = tle_total, group = frag_id, color = frag_id)) +
      facet_wrap(~ coral_id, scales = "free", nrow = 2) +
      geom_line(lwd = 0.1) +
      geom_line(data = p) + 
      geom_point() +
      labs(title = paste(as.character(n), "Tree", t), x = "", y = "Total Linear Extension (cm)") +
      theme(axis.text.x = element_text(angle = 45, hjust = 1))
    }))
...
summary(cont)
cld(cont)
cld(contrast(lsm, "pairwise", by = "coral_id"))
